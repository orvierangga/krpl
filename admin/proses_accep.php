...
date_default_timezone_set('Asia/Jakarta');
  ...
  if (mysqli_num_rows($cek) !=0) {
		$edit =  mysqli_query($koneksi, "UPDATE `permintaan_produk` set `status_perm`='Diterima' where `kd_perm`='$udah'") or die(mysqli_error());
			...
			if($edit) {
			echo '<script type="text/javascript">alert("Data Berhasil Diterima") </script>';
			mysqli_query($koneksi, "INSERT INTO `produk_keluar`(`kd_produk`,`jumlah_prdk`,`keperluan`,`tanggal_keluar`,`status`,`id_petugas`) 
